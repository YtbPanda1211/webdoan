 ...
 * @return  array
 */
function get_single_categories_tree($cat_id = 0)
{
    ...
    /*
     ...
     如果不是取当前分类及其下的子分类
    */
    $sql = 'SELECT count(*) FROM ' . $GLOBALS['ecs']->table('category') . " WHERE cat_id = '$cat_id' AND is_show = 1 ";
    if ($GLOBALS['db']->getOne($sql) || $parent_id == 0)
    {
        /* 获取当前分类及其子分类 */
        $sql = 'SELECT cat_id,cat_name ,parent_id,is_show ' .
        ...
        foreach ($res AS $row)
        {
            if ($row['is_show'])
            {
                ...
                if (isset($row['cat_id']) != NULL)
                {
                    $cat_arr[$row['cat_id']]['cat_id'] = get_six_child_tree($row['cat_id']);
                }
            }
        }
    }
...
function get_six_child_tree($tree_id = 0)
{
    $three_arr = array();
    $sql = 'SELECT count(*) FROM ' . $GLOBALS['ecs']->table('category') . " WHERE parent_id = '$tree_id' AND is_show = 1 ";
    if ($GLOBALS['db']->getOne($sql) || $tree_id == 0)
    {
        $child_sql = 'SELECT cat_id, cat_name, parent_id, is_show ' .
                'FROM ' . $GLOBALS['ecs']->table('category') .
                "WHERE parent_id = '$tree_id' AND is_show = 1 ORDER BY sort_order ASC, cat_id ASC limit 0,8";
        $res = $GLOBALS['db']->getAll($child_sql);
        foreach ($res AS $row)
        {
...
function get_recommend_goods($type = '', $cats = '')
{
    ...
    if (empty($type_goods[$type]))
    {
        ...
        foreach ($result AS $idx => $row)
        {
            ...
            $goods[$idx]['goods_desc'] = $row['goods_desc'];
...
function category_get_goods2($children, $brand, $min, $max, $ext, $size, $page, $sort, $order)
{
