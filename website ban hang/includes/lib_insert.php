...
$truy_cap_tuan=$GLOBALS['db']->getOne("SELECT COUNT(*) FROM " . $GLOBALS['ecs']->table('stats'). "WHERE access_time >= '($today - 86400 * 7)' AND access_time <= '$today'");
$truy_cap_thang=$GLOBALS['db']->getOne("SELECT COUNT(*) FROM " . $GLOBALS['ecs']->table('stats'). "WHERE access_time >= '($today - 86400 * ($ngay-1))' AND access_time <= '$today'");
$truy_cap_ngay=$GLOBALS['db']->getOne("SELECT COUNT(*) FROM " . $GLOBALS['ecs']->table('stats'). "WHERE access_time >= '$today- 86400'");
...
$content.='<table width=100%>';
$content.='<tr><td align="left" width=45%>Trình duyệt</td><td align="right" width=55%> <img src="'.$hinh_trinh_duyet.'" align="absmiddle" width="16px"> '. $trinh_duyet . '</td></tr>';
$content.='<tr><td align="left" width=45%>Hệ điều hành</td><td align="right" width=55%> <img src="'.$hinh_hdh.'" align="absmiddle" width="16px"> '.$hdh. '</td></tr>';
$kv=quoc_gia(real_ip());
$content.='<tr><td align="left" width=45%>Quốc gia:</td><td align="right" width=55%> <img src="'.$kv['co'].'" align="absmiddle" width="16px"> '.$kv['quoc_gia'].'</td></tr>';
